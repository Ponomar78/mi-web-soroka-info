...
async def _collect_wifi_async(udid: str | None = None) -> dict[str, Any]:
    from pymobiledevice3.services.diagnostics import DiagnosticsService

    lockdown = await _lockdown(udid)
    diag = DiagnosticsService(lockdown)
    try:
        raw = await diag.get_wifi()
        return {"collected_at": _utc_now(), "raw": raw}
    except Exception as exc:
        return {"error": str(exc), "collected_at": _utc_now()}


def collect_wifi(udid: str | None = None) -> dict[str, Any]:
    return _run(_collect_wifi_async(udid))
...
async def _full_scan_async(
    ...
    console.print("[bold]1/5 Device info…[/bold]")
    report["info"] = await _collect_info_async(udid)

    console.print("[bold]2/5 Battery…[/bold]")
    report["battery"] = await _collect_battery_async(udid)

    console.print("[bold]3/5 Wi‑Fi diagnostics…[/bold]")
    report["wifi"] = await _collect_wifi_async(udid)

    console.print("[bold]4/5 Apps…[/bold]")
    report["apps"] = await _collect_apps_async(udid)

    crash_dir: Path | None = None
    if pull_crash:
        console.print("[bold]5/5 Crash + panic-full…[/bold]")
        crash_dir = out_dir / f"crashes_{stamp}"
        report["crashes"] = await _pull_crashes_async(crash_dir, udid=udid)
        if report["crashes"].get("error"):
            crash_dir = None
        else:
            crash_dir = Path(report["crashes"]["path"])
    else:
        report["crashes"] = {"skipped": True}

    report["nfc_hints"] = try_nfc_related_hints(report.get("info") or {})
    report["advice"] = build_service_advice(
...
def render_html(report: dict[str, Any]) -> str:
    ...
    hyp_li = "".join(f"<li>{esc(h)}</li>" for h in (advice.get("hypotheses") or []))
    bat_tips = "".join(f"<li>{esc(h)}</li>" for h in (advice.get("battery_tips") or []))
    wet_li = "".join(f"<li>{esc(h)}</li>" for h in (advice.get("always_show_sound_wet") or []))
    panic_full = "".join(f"<li><code>{esc(p)}</code></li>" for p in (advice.get("panic_full_files") or [])[:20])
    apps_preview = "".join(
        f"<li>{esc(a.get('name'))} — {esc(a.get('bundle_id'))} ({esc(a.get('version'))})</li>"
        for a in apps[:40]
        if "error" not in a
    )
    cats = ", ".join(advice.get("matched_categories") or []) or "немає збігів у логах"
...
<head>
  ...
  <style>
    ...
    .warn {{ color: #b60; }}
    .important {{ border-left: 4px solid #c40; padding-left: .8rem; }}
  </style>
</head>
<body>
  <h1>iPhone Service Diagnostics</h1>
  <p>Зібрано: <strong>{esc(report.get("collected_at"))}</strong></p>
  <p>{esc(advice.get("device_line"))}</p>

  <div class="card important">
    <h2>Ймовірні проблеми (евристика)</h2>
    <p>Категорії з логів: <strong>{esc(cats)}</strong></p>
    <ul>{hyp_li or "<li>Явних збігів у crash/panic не знайдено — дивіться ручні тести на телефоні.</li>"}</ul>
    <h3>Батарея</h3>
    <ul>{bat_tips or "<li>—</li>"}</ul>
  </div>

  <div class="card important">
    <h2>Немає звуку / був мокрий</h2>
    <ul>{wet_li}</ul>
  </div>

  <div class="card">
    <h2>Panic-full</h2>
    <ul>{panic_full or "<li>panic-full файлів не знайдено в зтягнутих логах.</li>"}</ul>
  </div>
  ...
  <div class="card">
    ...
    <p>Орієнтовне здоров'я: <strong>{esc(battery.get("estimated_health_percent"))}%</strong>
       <span class="warn">(оцінка з IORegistry)</span></p>
    <table>{rows_bat}</table>
  </div>

  <div class="card">
    <h2>Wi‑Fi (diagnostics relay)</h2>
    <pre>{esc(wifi.get("error") or wifi.get("raw") or wifi)}</pre>
  </div>
  ...
  <div class="card">
    <h2>NFC / підказки</h2>
    <ul>{hints_li}</ul>
    <p><strong>Face ID / NFC / динамік / GSM «на слух»</strong> — кнопки в апці на iPhone (ios-app), не по USB.</p>
  </div>
